...
//
//  Desc:   class to encapsulate steering behaviors for a CVehicle
//
...
private:  
  ...
  CVehicle*     m_pTargetAgent1;
  CVehicle*     m_pTargetAgent2;

  //the current _vTarget
  Vector2D			m_vTarget;

  //length of the 'detection box' utilized in CObstacle avoidance
  ...
  //pointer to any current CPath
  CPath*        m_pPath;
	bool					m_bPathActive;

  //the distance (squared) a CVehicle has to be from a CPath waypoint before
  ...
  //Arrive makes use of these to determine how quickly a CVehicle
  //should decelerate to its _vTarget
  ...
  //this behavior moves the agent towards a _vTarget position
  Vector2D Seek(Vector2D TargetPos);

  //this behavior returns a vector that moves the agent away
  //from a _vTarget position
  Vector2D Flee(Vector2D TargetPos);

  //this behavior is similar to seek but it attempts to arrive 
  //at the _vTarget position with a zero velocity
  ...
  Vector2D Pursuit(CVehicle* agent);

  //this behavior maintains a position, in the direction of offset
  //from the _vTarget CVehicle
  Vector2D OffsetPursuit(CVehicle* agent, Vector2D offset);

  //this behavior attempts to evade a pursuer
  Vector2D Evade(CVehicle* agent);
  ...
  Vector2D ObstacleAvoidance(std::vector<CObstacle*>& obstacles);

  //this returns a steering force which will keep the agent away from any
  //walls it may encounter
  Vector2D WallAvoidance(std::vector<CWall3D> &walls);

  //given a series of Vector2Ds, this method produces a force that will
  //move the agent along the waypoints in order
  Vector2D FollowPath();

  //this results in a steering force that attempts to steer the CVehicle
  //to the center of the vector connecting two moving agents.
  Vector2D Interpose(CVehicle* VehicleA, CVehicle* VehicleB);

  //given another agent position to hide from and a list of BaseEntitys this
  //method attempts to put an CObstacle between itself and its opponent
  Vector2D Hide(CVehicle* hunter, std::vector<CObstacle*>& obstacles);

	//	if controlled, this function is used
	Vector2D Controlled();

  // -- Group Behaviors -- //
	// ------------------------ For Vehicles-----------------------------
  Vector2D Cohesion(std::list<CVehicle*> &neighbors);  
  Vector2D Separation(std::list<CVehicle*> &neighbors);
  Vector2D Alignment(std::list<CVehicle*> &neighbors);
  //the following three are the same as above but they use cell-space
  //partitioning to find the neighbors
  Vector2D CohesionPlus(std::list<CVehicle*> &neighbors);
  Vector2D SeparationPlus(std::list<CVehicle*> &neighbors);
  Vector2D AlignmentPlus(std::list<CVehicle*> &neighbors);
  ...
  //side of an CObstacle to the pursuer
...
public:
  ...
  void      SetTargetAgent1(CVehicle* Agent){m_pTargetAgent1 = Agent;}
  void      SetTargetAgent2(CVehicle* Agent){m_pTargetAgent2 = Agent;}
  ...
  void      SetPath(std::list<Vector2D> new_path);
	CPath*    GetPath(){return m_pPath;}
	bool			IsPathActive(){return m_bPathActive;}
	void			OnPathActive(){m_bPathActive = true;}
	void			OffPathActive(){m_bPathActive = false;}
  ...
  void PursuitOn(CVehicle* v){m_iFlags |= pursuit; m_pTargetAgent1 = v;}
  void EvadeOn(CVehicle* v){m_iFlags |= evade; m_pTargetAgent1 = v;}
  ...
  void FollowPathOn(){m_iFlags |= follow_path;}
	void ControlledOn(){m_iFlags |= controlled;}
  void InterposeOn(CVehicle* v1, CVehicle* v2){m_iFlags |= interpose; m_pTargetAgent1 = v1; m_pTargetAgent2 = v2;}
  void HideOn(CVehicle* v){m_iFlags |= hide; m_pTargetAgent1 = v;}
  void OffsetPursuitOn(CVehicle* v1, Vector2D offset){m_iFlags |= offset_pursuit; m_vOffset = offset; m_pTargetAgent1 = v1;}  
  ...
  void FollowPathOff(){if(On(follow_path)) m_iFlags ^=follow_path;}
	void ControlledOff(){if(On(controlled)) m_iFlags ^=controlled;}
  ...
  bool isFollowPathOn(){return On(follow_path);}
	bool isControlledOn(){return On(controlled);}
  ...
  const std::vector<Vector2D>& GetFeelers()const{return m_Feelers;}
	...
	double GetViewDistance(){return m_dViewDistance;}
	void	 SetViewDistance(double _dDist){m_dViewDistance = _dDist;}
