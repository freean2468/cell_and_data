...
void TagNeighbors(const T& entity, conT& ContainerOfEntities, double radius){
       ...
       ++curEntity)
  {
    ...
    Vector2D to = (*curEntity)->GetPos2D() - entity->GetPos2D();
...
void EnforceNonPenetrationConstraint(const T& entity, const conT& ContainerOfEntities){
       ...
       ++curEntity)
  {
    ...
    Vector2D ToEntity = entity->GetPos2D() - (*curEntity)->GetPos2D();
    ...
    //ToEntity vector   
    double AmountOfOverLap = (*curEntity)->GetRadius() + entity->GetRadius() - DistFromEachOther;

    if (AmountOfOverLap >= 0){
      //move the entity a distance away equivalent to the amount of overlap.
      entity->SetPos(entity->GetPos2D() + (ToEntity/DistFromEachOther) * AmountOfOverLap);
    }
  }//next entity
}
                                               ...
                                               double      range = MaxDouble)
{
  ...
  for (it=entities.begin(); it != entities.end(); ++it){
    ...
    if ( ((*it)->GetID() == the_one_to_ignore) || (Vec2DDistanceSq((*it)->GetPos2D(), A) > range*range) ){
      continue;
    }

    //if the distance to AB is less than the entities bounding radius then
    //there is an intersection so add it to hits
    if (DistToLineSegment(A, B, (*it)->GetPos2D()) < (*it)->GetRadius()){
      hits.push_back(*it);
    }
                                          ...
                                          double       range = MaxDouble)
{
  ...
  //iterate through all entities checking against the line segment AB
  for (it=entities.begin(); it != entities.end(); ++it){
    double distSq = Vec2DDistanceSq((*it)->GetPos2D(), A);

    //if not within range or the entity being checked is the_one_to_ignore
    //just continue with the next entity
    if ( ((*it)->GetID() == the_one_to_ignore) || (distSq > range*range) ){
      continue;
    }
