...
#include "StreamUtilityFunctions.h"
#include <string>
...
void ErrorBox(std::string& _rMsg)
{
  MessageBox(NULL, _rMsg.c_str(), "Error", MB_OK);
}

void ErrorBox(char* _pstrMsg)
{
  MessageBox(NULL, _pstrMsg, "Error", MB_OK);
}
...
// Char->WChar
WCHAR* CharToWChar(const char* _pstrSrc){
	assert(_pstrSrc);
	size_t t_nLen = strlen(_pstrSrc)+1;

	wchar_t* t_pwstrTemp = (LPWSTR)malloc(sizeof(wchar_t)*t_nLen);

	size_t ConvertedChars = 0;

	mbstowcs_s(&ConvertedChars, t_pwstrTemp, t_nLen, _pstrSrc, _TRUNCATE);
