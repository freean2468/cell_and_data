...
// Desc: Returns the DirectX SDK media CPath
...
HRESULT DXUtil_GetDXSDKMediaPathCch( TCHAR* strDest, int cchDest )
{
    ...
    lResult = RegQueryValueEx( hKey, _T("DX9SDK Samples CPath"), NULL,
...
// Desc: Returns a valid CPath to a DXSDK media file
...
HRESULT DXUtil_FindMediaFileCch( TCHAR* strDestPath, int cchDest, TCHAR* strFilename )
{
    ...
    // Build full CPath name from strFileName (strShortName will be just the leaf filename)
    ...
    // first try to find the filename given a full CPath
    ...
    // next try to find the filename in the current working directory (CPath stripped)
    ...
    // On failure, just return the file as the CPath
...
VOID DXUtil_LaunchReadme( HWND hWnd )
{
    ...
    // Get the exe name, and exe CPath
    ...
    if( strLastSlash )
    {
        ...
        // Chop the exe name from the exe CPath
        *strLastSlash = 0;
