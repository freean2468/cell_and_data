...
struct Vector2D{
  ...
  const Vector2D& operator=(const D3DXVECTOR3 &rhs){
		x = rhs.x;
		y = rhs.y;
...
inline Vector2D operator+(const Vector2D &lhs, const Vector2D &rhs);
inline Vector2D operator/(const Vector2D &lhs, double _dVal);
...
//  is operating upon. (like the CPath of a ball bouncing off a wall)
//------------------------------------------------------------------------
...
inline void WrapAround(Vector2D &pos, int MaxX, int MaxY){
  ...
  if (pos.x < -MaxX/2)	{pos.x = (double)MaxX/2;}

  if (pos.y < -MaxY/2)				{pos.y = (double)MaxY/2;}
...
//
//  returns true if the _vTarget position is in the field of view of the entity
//  positioned at posFirst facing in facingFirst
//-----------------------------------------------------------------------------
inline bool isSecondInFOVOfFirst(Vector2D posFirst, Vector2D facingFirst, Vector2D posSecond, double fov){
